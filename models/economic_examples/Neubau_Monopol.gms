...
problem Peak Load Pricing or Uncertainty? / 0.5   /
...
profit(i,s).. -problem*[ alpha(s) -  2*beta*q(i,s)   -c ] + lambda(i,s)- beta*psi(s) + beta*jota(s) =g= 0;
* beta*sum(j, q(j,s))    - jota(i)*problem*((alpha(s) - beta*sum(j,q(j,s)))-c)
...
nonneg1(s)..    problem*(alpha(s)- beta*sum(j,q(j,s)) - c) - gamma  =g= 0  ;
*
capacities..    capaci =e= sum(j, K1(j)) ;

model simple / profit.q, invest.In,price, capacity.K1, constraint.lambda, state , pricecap.psi,nonneg1.jota,  capacities /
*
solve simple using mcp;

display q.l, In.l, K1.l, lambda.l, u.l, capaci.l, psi.l , p.l  , jota.l