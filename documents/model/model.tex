...
$\alpha(m)$ and $\beta(m)$ are the intercept and the slope of the inverse demand function in different market states which could be very high, high, intermediate etc.. $q_{i,k}^m$ is the quantity produced with each technology by each player in each market state and $K_i^k$ is the corresponding capacity. $K_i^t$ is now given for each player as we are solving for the short run equilibrium. $c^k$ are the short run variable costs which are the same for every producer. The parameter $\nu_m $ says how often a certain demand state occurs. This parameter is used as a weight for the shadow price of capacity which we obtain. We derived the Karush Kuhn Tucker (KKT) conditions to obtain a Mixed Complementarity Problem (MCP) and solved it by using the PATH Solver in GAMS.
...
\begin{gather}
	...
	+ \epsilon \sum_s p_s \sum_m v_m \times \\ \nonumber \left[ (\alpha_m^s-\beta_m^s \sum_i\sum_k q_{i,k}^{s,m,2}) \sum_k q_{i,k}^{s,m,2} - \sum_k c_k q_{i,k}^{s,m,2}  \right]   \\  \nonumber 
