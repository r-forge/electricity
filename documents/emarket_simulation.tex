\documentclass[a4paper,11pt]{article}
\usepackage{a4wide}
\usepackage[ansinew]{inputenc}
\usepackage{graphicx}
\usepackage{bm}
\usepackage{natbib}
\usepackage{longtable}
\usepackage{rotating}
\usepackage{pdflscape}
%\usepackage{pdfdraftcopy}


%\usepackage{jurabib}
   %\jurabibsetup{  
     %authorformat=and  
    % commabeforerest,  
    % titleformat=colonsep,  
    % bibformat=tabular  
   %}  

\usepackage[bookmarks=true, bookmarksopen=true,
                bookmarksnumbered=true, colorlinks,citecolor = blue,
                filecolor=blue, linkcolor=blue, urlcolor=blue,
                plainpages=false,hyperindex=true]{hyperref}
\hypersetup{
        pdftitle={},
        pdfauthor={Anton Burger and Robert Ferstl},
        pdfsubject={},
        pdfkeywords={},
        pdfcreator={},
        pdfproducer={}
     }

%\usepackage{natbib}
%\usepackage{dingbat}
\usepackage{amssymb,amsmath,amstext}
%\usepackage{manfnt}

\setlength{\parindent}{0.0cm}                      % Absatzeinrückungen
\setlength{\parskip}{1.5ex plus 0.5ex minus 0.5ex} % Absatzabstände
%\renewcommand{\arraystretch}{1.5}                 % Zeilenabstand in Tabellen


\begin{document}
\title{Generation Capacity Investment in \\Electricity Markets under Uncertainty\thanks{available online at \href{http://www.ssrn.com}{http://www.ssrn.com}}\\\vspace{0.3cm}\emph{Draft version}}
\author{Anton Burger\thanks{\href{http://www.wu-wien.ac.at/regulierung}{Institute for Regulatory Economics, Vienna University of Economics and Business Administration,} \href{mailto:anton.burger@wu-wien.ac.at}{anton.burger@wu-wien.ac.at}}\hspace{1cm} Robert Ferstl\thanks{\href{http://www-finanzierung.uni-regensburg.de/}{Department of Finance, University of Regensburg,} \href{mailto:robert.ferstl@wiwi.uni-regensburg.de}{robert.ferstl@wiwi.uni-regensburg.de}}}
\maketitle
\abstract{
The paper discusses game theoretic models for generation capacity investment decisions in a deregulated electricity market. The possible problem formulations involve open-loop, closed-loop and $S$-adapted Cournot equilibria. We give an example for the Austrian/German wholesale electricity market and analyze the welfare implications of these different strategic decision support models.
\\
\\
\textbf{Keywords:} Electricity markets, Investment decisions, Stochastic oligopoly model, Sample path optimization, Stochastic programming
\\
\\
\textbf{JEL Codes:} D43, L51
}

\input{introduction/introduction.tex}
%\input{games/games.tex}
\input{germandata/germandata.tex}
\input{capacity/capacity.tex}
\input{model/model.tex}
%\input{data/data.tex}
%\input{ideas/ideas.tex}



%------------------------------------------------------

\markboth{References}{References} %\frenchspacing
\bibliographystyle{chicago}
%\nocite{*}
\bibliography{microsim}

\appendix
\newpage
\section*{Appendix}
\input{appendix/literature.tex}
\input{appendix/instcapacities.tex}
\end{document}

%%% Local Variables: 
%%% mode: latex
%%% TeX-master: t
%%% End: 
